mod test {
    ...
    use bulletproofs_rs::random_ec_points::{generate_n_random_points, generate_random_field_element};
    use rand::Rng;
    ...
    fn test_fold_group() {
        ...
        // Verify first folded element
        let expected_first = (original_input[0] * u + original_input[1] * u.inverse().unwrap()).into_affine();
        assert_eq!(result[0], expected_first);
        
        // Verify second folded element (with padding)
        let expected_second = (original_input[2] * u + G1Affine::zero() * u.inverse().unwrap()).into_affine();
        assert_eq!(result[1], expected_second);
    }
    ...
    fn test_manual_verification() {
        ...
        let folded_g = ((g_vec[0] * u_inv).into_affine() + (g_vec[1] * u).into_affine()).into_affine();
